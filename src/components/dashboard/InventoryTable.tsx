    ...
    return (
        <Card className="glass border-transparent text-slate-800 min-h-[600px] shadow-sm">
            ...
            <CardContent className="pt-6">
                <div className="rounded-xl border border-slate-200 bg-white/50 h-[650px] overflow-y-auto relative outline-none rounded-t-xl custom-scrollbar overflow-x-hidden">
                    <table className="w-full caption-bottom text-[13px] relative">
                        <thead className="sticky top-0 bg-white/95 backdrop-blur-sm z-20 border-b border-slate-200 shadow-sm [&_tr]:border-b-0">
                            <TableRow className="hover:bg-transparent">
                                <th className="h-10 px-2 text-left align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[120px]" onClick={() => handleSort('ide')}>
                                    IDE {getSortIcon('ide')}
                                </th>
                                <th className="h-10 px-2 text-left align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[80px]" onClick={() => handleSort('raza')}>
                                    Raza {getSortIcon('raza')}
                                </th>
                                <th className="h-10 px-2 text-left align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[90px]" onClick={() => handleSort('padre')}>
                                    Padre {getSortIcon('padre')}
                                </th>
                                <th className="h-10 px-2 text-right align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[80px]" onClick={() => handleSort('currentWeight')}>
                                    Peso {getSortIcon('currentWeight')}
                                </th>
                                <th className="h-10 px-2 text-right align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[85px]" onClick={() => handleSort('currentGdm')}>
                                    GDM Act. {getSortIcon('currentGdm')}
                                </th>
                                <th className="h-10 px-2 text-right align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[85px]" onClick={() => handleSort('averageGdm')}>
                                    GDM Prom. {getSortIcon('averageGdm')}
                                </th>
                                <th className="h-10 px-2 text-left align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[120px]" onClick={() => handleSort('reproductiveState')}>
                                    Reprod. {getSortIcon('reproductiveState')}
                                </th>
                                <th className="h-10 px-2 text-right align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[80px]" onClick={() => handleSort('daysToTarget')}>
                                    D/Obj. {getSortIcon('daysToTarget')}
                                </th>
                                <th className="h-10 px-2 text-right align-middle font-semibold whitespace-nowrap text-slate-500 cursor-pointer select-none min-w-[70px] pr-4" onClick={() => handleSort('scoreTotal')}>
                        ...
                        <TableBody>
                            ...
                            sortedAnimals.map((animal, idx) => (
                                <TableRow key={animal.ide} className={`border-b border-slate-100 hover:bg-indigo-50/20 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'}`}>
                                    ...
                                    <TableCell
                                        className="font-bold text-indigo-600 cursor-pointer hover:underline px-2 max-w-[130px]"
                                        onClick={() => {
                                    ...
                                    <TableCell className="text-slate-600 px-2 whitespace-nowrap">{animal.raza}</TableCell>
                                    <TableCell className="text-slate-600 font-medium px-2 whitespace-nowrap">{animal.padre}</TableCell>
                                    {/* Peso: right-aligned, no decimals, unit label */}
                                    <TableCell className="text-slate-800 font-semibold px-2 text-right tabular-nums">
                                        {animal.currentWeight !== null ? animal.currentWeight : '-'}
                                        {animal.currentWeight !== null && <span className="text-slate-400 text-[10px] ml-0.5">kg</span>}
                                    </TableCell>
                                    ...
                                    <TableCell className="px-2">
                                        {animal.reproductiveState ? (
                                            <Badge variant="outline" className={`font-medium border-transparent text-[11px] leading-tight px-1.5 py-0
                                    ...
                                    <TableCell className="px-2 text-right tabular-nums">
                                        {animal.daysToTarget !== null ? (
                                            <span className={`${animal.alertYellow ? 'text-amber-600 font-bold' : 'text-slate-600'}`}>
                                                {animal.daysToTarget} <span className="text-[10px] text-slate-400 font-normal">d</span>
                                            </span>
                                        ) : <span className="text-slate-400">-</span>}
                                    </TableCell>
                                    <TableCell className="text-right px-2 pr-4">
                                        <div className="flex items-center justify-end gap-1.5 text-slate-800">
                                            {animal.alertRed && <span className="w-2 h-2 rounded-full bg-orange-500 shadow-sm shadow-orange-500/50 shrink-0" title="Alerta Crítica"></span>}
                                            {animal.alertYellow && !animal.alertRed && <span className="w-2 h-2 rounded-full bg-amber-500 shadow-sm shadow-amber-500/50 shrink-0" title="Alerta Retraso IATF"></span>}
                                            <span className="font-bold text-base tabular-nums">{animal.scoreTotal}</span>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ))
